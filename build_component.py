...
def main():
  ...
  try:
	config_file = open("scripts/config_build.tx")
  except IOError:
	print("ERROR~~Cannot find config_build.txt. You need that as build will copy all the files necessary to build the particular Seattle Component into the target directory.")
  	print("Config_build.txt is needed in order to fetch the list of files and directories to copy to target")
	sys.exit(0) 
